# based on: https://github.com/tinygrad/tinygrad/blob/5d5ead78dad797b0b914377d5d568efd377235ca/examples/stable_diffusion.py
...
from tinygrad.helpers import Timing, Context, getenv, tqdm, flatten, profile_marker
...
class ResnetBlock:
  def __init__(self, in_channels, out_channels=None):
    ...
    self.conv1 = Conv2d(in_channels, out_channels, 3, padding=1) # type: ignore
    self.norm2 = GroupNorm(32, out_channels) # type: ignore
    self.conv2 = Conv2d(out_channels, out_channels, 3, padding=1) # type: ignore
    self.nin_shortcut = Conv2d(in_channels, out_channels, 1) if in_channels != out_channels else lambda x: x # type: ignore
  ...
  def __init__(self, path:str, version:str|None=None, pretrained:str|None=None): # loads only safetensors
    ...
    if not version:
      ...
      if pretrained:
        ...
        weights: dict[str,Tensor] = {k.replace('cond_stage_model.', '', 1):v for k,v in torch_load(pretrained)['state_dict'].items() if k.startswith('cond_stage_model.')} # type: ignore
        ...
        if version == 'v2-mlperf-eval':
          ...
          weights = {k.replace('first_stage_model.', '', 1):v for k,v in torch_load(pretrained)['state_dict'].items() if k.startswith('first_stage_model.')} # type: ignore
...
class StableDiffusionText2ImagePipeline(Text2ImagePipeline[StableDiffusion]):
  ...
  def generate(self, prompt, nevative_prompt, steps, seed, guidance) -> Tensor:
    ...
    context = self.model.cond_stage_model.transformer.text_model(encoded_prompt).realize() # type: ignore
    ...
    alphas = self.model.alphas_cumprod[Tensor(timesteps)]
    ...
    with Context(BEAM=getenv('LATEBEAM')):
      for index, timestep in (t:=tqdm(list(enumerate(timesteps))[::-1])):
        ...
        # enabled=args.timing
        with Timing('step in ',  on_exit=lambda _: f', using {GlobalCounters.mem_used/1e9:.2f} GB'):
    ...
    x = self.model.decode(latent).realize()
    print(x.shape)
    return x
